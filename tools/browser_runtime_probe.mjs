...
if (typeof WebSocket !== "function") {
  throw new Error("browser runtime probe requires Bun with global WebSocket")
}
...
try {
  ...
  assert(
    deepLink.rowSelected === "true" && deepLink.detailVisible,
    "deep link selected wrong claim",
  )
  ...
  await evaluate(
    "Array.from(document.querySelectorAll('[data-evidence-detail]'))" +
      ".find((detail) => detail.dataset.evidenceDetail === " +
      claimLiteral +
      ").focus()",
  )
  await press("Tab")
  ...
  await evaluate(
    "Array.from(document.querySelectorAll('[data-evidence-open]'))" +
      ".find((button) => button.dataset.evidenceOpen === " +
      claimLiteral +
      ").click()",
  )
  await waitFor(
  ...
  assert(
    emptyFilter.emptyVisible &&
      emptyFilter.visibleRows === 0 &&
      emptyFilter.visibleDetails === 0 &&
      emptyFilter.claim === null,
  ...
  process.stdout.write(
    "browser runtime probe: evidence URL, filters, focus, close, and tour states OK\n",
  )
} finally {
  session.close()
}
