...
- Stack = Linux + Bun/TypeScript + repository-owned vanilla HTML/CSS/JS; pnpm/Node pins = `package.json`, `pnpm-lock.yaml`, `.node-version`. Bootstrap = `pnpm install --frozen-lockfile`.
- Authoring boundary = untrusted declarative JSON → schema/evidence validation → trusted template compilation → validated atomic publication. Codex supplies copy/data only; executable UI belongs in tracked templates.
- Every localized field carries nonempty `ja` + `en`; initial locale = Japanese. Structural validity proves neither translation parity nor claim truth. Review whole bundles, including verbatim citations, before commit/push.
- Standalone artifacts retain checkout-local compatibility; hosted `--artifact-root` stays outside source checkouts. Linux `renameat2` directory exchange owns publication; checkout-bound launcher = `bin/preview`.
- Compiler canaries = `previews/lean-cds/` + `previews/in-progress/`. Renderer/template changes → `pnpm canaries:sync`; byte-equivalence check = `pnpm canaries:check` (`tools/sync-canaries.ts`). Current-source model validation remains separate from canary compilation.
- Deterministic gate = `just ci` (`justfile`, `package.json`): canaries, formatting, lint, types, Bun/runtime tests, template lint, ShellCheck, diff hygiene; no Codex. Focused tests = `just test`.
- Browser gate = `just browser-probe`; real Chromium interactions = `PREVIEW_PROBE_CHROMIUM=1 just browser-probe`. Fixtures = `testdata/valid` + `testdata/source`; browser override = `PREVIEW_PROBE_CHROMIUM_BIN`. Probe mechanics + interpretation = `README.md` → Development.
